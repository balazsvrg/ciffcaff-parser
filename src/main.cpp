...
int main(int argc, char* argv[]) {
    ...
    if (mode == "-ciff") {
        ...
        if (!ciff_image.parse_image()) {
            std::cerr   << "Failed to parse CIFF file: " 
                        << image_file_path 
                        << std::endl;
            return -1;
        }
        ...
        std::string output_filename = 
            image_file_path.substr(0, image_file_path.find_last_of('.')) + ".jpeg";

        // Encode CIFF image as JPEG
        JpegEncoder jpeg_encoder(output_filename, 
                                 ciff_image.get_header().width, 
                                 ciff_image.get_header().height, 
                                 JPEG_QUALITY);

        if (!jpeg_encoder.encode(ciff_image.get_data())) {
            std::cerr   << "Failed to encode CIFF image to JPEG: " 
                        << output_filename 
                        << std::endl;
            return -1;
        }
        ...
        if (!caff_image.parse_image()) {
            std::cerr   << "Failed to parse CAFF file: " 
                        << image_file_path 
                        << std::endl;
            return -1;
        }
        ...
        if (animations.empty()) {
            std::cerr   << "No animations found in CAFF file: " 
                        << image_file_path 
                        << std::endl;
            return -1;
        }
        ...
        JpegEncoder jpeg_encoder(output_filename, 
                                 ciff_frame.get_header().width, 
                                 ciff_frame.get_header().height, 
                                 JPEG_QUALITY);

        if (!jpeg_encoder.encode(ciff_frame.get_data())) {
            std::cerr   << "Failed to encode CIFF frame to JPEG: " 
                        << output_filename 
                        << std::endl;
            return -1;
        }
